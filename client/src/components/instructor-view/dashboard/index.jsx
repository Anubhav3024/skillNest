    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white p-4 rounded-xl border border-[#0d694f]/10 shadow-3d-orange backdrop-blur-md">
          <p className="text-[9px] font-bold text-[#0d694f] uppercase tracking-wider mb-1">{payload[0].payload.name}</p>
        ...
        <div className="flex flex-col md:flex-row items-center gap-10 relative z-10">
          ...
          <div className="flex-1 space-y-8 text-center md:text-left">
            <div className="space-y-1">
              <div className="text-[11px] font-bold text-[#ff7e5f]  opacity-80">Certified Educator</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 pt-4 border-t border-[#0d694f]/5">
              <div className="space-y-1">
                <div className="text-[10px] font-bold text-[#ff7e5f] ">Experience</div>
                <div className="text-[14px] font-headline font-bold text-[#0d694f] tracking-tight leading-normal">
                  {auth?.user?.experience || '2'}
                </div>
              </div>
              <div className="space-y-1">
                <div className="text-[10px] font-bold text-[#ff7e5f] ">Philosophy Manifest</div>
      ...
      {!isFirstTime && (
        <>
            ...
            {stats.map((stat, index) => (
              ...
              >
                ...
                <div className="space-y-1">
                  ...
                  <div className="text-[10px] font-bold text-muted-foreground/50 ">{stat.change}</div>
                </div>
              </motion.div>
            ))}
          </div>
          ...
          {trayectoryData.length > 0 && (
            <motion.div variants={itemVariants} className="bg-white p-8 rounded-[2.5rem] border border-[#0d694f]/5 shadow-3d relative overflow-hidden">
                <div className="flex items-center justify-between mb-8">
                  <div>
                    <h3 className="text-lg font-headline font-bold text-[#0d694f] tracking-tight">Financial Trajectory</h3>
                    <p className="text-[10px] text-muted-foreground font-bold  opacity-40">Monthly Performance Manifest</p>
                  </div>
                ...
                <div className="h-48">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={trayectoryData}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#0d694f05" />
                      <XAxis dataKey="name" hide />
                      <YAxis hide />
                      <Tooltip content={<CustomTooltip />} />
                      <Bar dataKey="revenue" radius={[10, 10, 0, 0]}>
                        {trayectoryData.map((entry, index) => (
                          <Cell key={`cell-${index}`} fill={index === trayectoryData.length - 1 ? "#0d694f" : "#0d694f10"} />
                        ))}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <motion.div variants={itemVariants} className="lg:col-span-8 bg-white p-8 rounded-2xl border border-[#0d694f]/5 shadow-3d overflow-hidden">
              <div className="flex items-center justify-between mb-8">
                <h3 className="text-lg font-headline font-bold text-[#0d694f] tracking-tight">Recent Transactions</h3>
                <button onClick={() => setActiveTab("earnings")} className="text-[9px] font-black text-muted-foreground/60 hover:text-[#ff7e5f]  flex items-center gap-2 transition-all group/all">
                ...
                <table className="w-full">
                  <thead>
                    <tr className="text-left border-b border-[#fcf8f1]">
                      <th className="pb-3 text-[10px] font-bold text-muted-foreground/40  opacity-60">Scholar</th>
                      <th className="pb-3 text-[10px] font-bold text-muted-foreground/40  opacity-60">Vault</th>
                      <th className="pb-3 text-[10px] font-bold text-muted-foreground/40  opacity-60 text-right">Valuation</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#fcf8f1]">
                    {analytics?.transactions?.slice(0, 5).map((payment, index) => (
                      <motion.tr key={payment._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.05 * index }} className="group hover:bg-[#fcf8f1]/50 transition-colors">
                        <td className="py-3 px-1">
                           <div className="text-[12px] font-headline font-bold text-[#0d694f] tracking-tight group-hover:text-[#ff7e5f] transition-colors">{capitalize(payment.userName)}</div>
                        </td>
                        <td className="py-3 px-1 text-[11px] text-muted-foreground font-medium italic truncate max-w-[150px]">{payment.courseTitle}</td>
                        <td className="py-3 px-1 text-right text-[12px] font-headline font-black text-[#0d694f]">₹{payment.coursePricing}</td>
                      </motion.tr>
                    ))}
                    {(!analytics?.transactions || analytics.transactions.length === 0) && (
                      <tr>
                        <td colSpan="3" className="py-8 text-center text-[10px] font-bold text-muted-foreground/30">No recent activity</td>
                      </tr>
                    )}
                  </tbody>
                </table>
            ...
            <motion.div variants={itemVariants} className="lg:col-span-4 bg-[#dfede9] p-8 rounded-2xl border border-[#0d694f]/10 shadow-3d">
              <h3 className="text-lg font-headline font-bold text-[#0d694f] mb-8  tracking-tight">Top Manifests</h3>
              <div className="space-y-6">
                {bestPerformingCourses.map((course, index) => (
                  <motion.div key={index} className="flex items-center justify-between group">
                    <div className="flex items-center gap-3">
                       <div className="w-8 h-8 rounded-lg bg-white flex items-center justify-center text-[10px] font-black text-[#0d694f] border border-[#0d694f]/5 group-hover:bg-[#0d694f] group-hover:text-white transition-all">{index + 1}</div>
                       <div className="text-[11px] font-headline font-bold text-[#0d694f] tracking-tight line-clamp-1 max-w-[120px]">{capitalize(course.title)}</div>
                    </div>
                    <div className="text-[11px] font-black text-[#ff7e5f]">{course.sales}</div>
                  </motion.div>
                ))}
              </div>
            </motion.div>
...
const OnboardingCard = ({ step, title, desc, buttonLabel, icon: Icon, onClick }) => (
  <motion.div 
  ...
  >
    <div className="space-y-4 relative z-10">
      <div className="flex items-center justify-between">
        ...
        <span className="text-2xl font-headline font-black text-[#0d694f]/10">{step}</span>
      </div>
      <div className="space-y-1">
        <h3 className="text-lg font-headline font-bold text-[#0d694f] tracking-tight">{title}</h3>
    ...
    >
      <Plus className="h-3 w-3 transition-transform group-hover/btn:rotate-90" /> {buttonLabel}
    </button>
