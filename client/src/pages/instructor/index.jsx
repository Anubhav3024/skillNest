...
const InstructorDashboardPage = () => {
  ...
  return (
    <div className="flex h-screen bg-[#fcf8f1] font-body text-slate-900 overflow-hidden smooth-scroll">
      {/* Sidebar */}
      <aside className="w-16 sm:w-60 bg-white/70 backdrop-blur-2xl border-r border-[#0d694f]/5 fixed top-0 left-0 h-screen z-50 flex flex-col pt-0 px-2 sm:px-4 pb-5 transition-all duration-500 shadow-3d">
        <motion.div 
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto custom-scrollbar pr-0 sm:pr-2">
          {menuItems.map((menuItem) => (
            ...
            >
              ...
              {activeTab === menuItem.value && (
                <motion.div layoutId="activeTabBadge" className="sm:ml-auto absolute sm:static right-2 top-1/2 -translate-y-1/2 w-1 h-1 rounded-full bg-[#ff7e5f] shadow-[0_0_8px_rgba(255,126,95,0.8)]"></motion.div>
              )}
      ...
      {/* Main Content */}
      <main className="flex-1 min-w-0 ml-16 sm:ml-60 overflow-y-auto overflow-x-hidden custom-scrollbar relative scroll-smooth">
        {/* Header */}
        <motion.header 
        ...
        >
          ...
          <div className="flex items-center justify-between sm:justify-start gap-3 sm:gap-6">
            ...
            >
              ...
              <span className="hidden sm:inline text-[12px] font-headline font-bold text-white tracking-tight">{auth?.user?.userName?.split(" ")[0]}</span>
            </div>
